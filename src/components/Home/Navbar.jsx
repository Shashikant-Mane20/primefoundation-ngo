          ...
          >
            {menuItems.map((item, index) => (
              <li key={index} className="relative group px-4 py-2 md:p-0">
                ...
                {item.dropdown && (
                  <ul
                    className={`absolute left-0 top-full bg-white text-black rounded shadow-md opacity-0 group-hover:opacity-100 hidden md:block transition-opacity duration-300`}
                  >
                    {item.dropdown.map((subItem, subIndex) => (
                      <li
                        key={subIndex}
                        className="py-2 px-4 hover:bg-gray-200 whitespace-nowrap"
                      >
